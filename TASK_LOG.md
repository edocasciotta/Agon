...
| P | UX improvements (branch `feat/ux-improvements`): **Quick wins** — (3) Reactivate establishments; (4) Reactivate inactive instructors + show inactive state on cards; (6) Memberships currency column translated; (7) Type column translated (recurring/credit pack); (10) Calendar filter label fixed. **Medium complexity** — (#1) Primary/secondary color customization in Settings with CSS var injection in App.tsx; (#5) Per-class booking windows (cancellation_window_hours, booking_open_hours_before, booking_close_hours_before) added to class_templates + UI in ClassTypes modal + booking router uses per-class window with global fallback; (#9) ID always visible on establishment/instructor cards + client table ID column; search by numeric ID in clients/instructors/establishments; Alembic migrations `d1e2f3a4b5c6` + `e2f3a4b5c6d7`; i18n keys added all 7 locales; 244 backend tests + 43 frontend tests pass |
