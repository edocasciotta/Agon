...
import { emailEventsApi } from '../../api/emailEvents'
...
export function EmailEventsPage() {
  ...
  const assignMutation = useMutation({
    mutationFn: ({ eventType, templateId }: { eventType: string; templateId: number | null }) =>
      emailEventsApi.assign(eventType, templateId),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['email-events'] })
    },
  })
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">{t('marketing.events')}</h1>
      </div>
            ...
            {events.map((ev) => (
              <tr key={ev.event_type}>
                ...
                <td className="px-4 py-3">
                  ...
                  >
                    <option value="">{t('marketing.defaultTemplate')}</option>
                    {templates.map((tpl) => (
