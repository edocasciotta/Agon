"""
SMS Event Assignments — /api/v1/sms/events
"""
...
router = APIRouter(prefix="/api/v1/sms/events", tags=["sms-events"])
...
def list_event_assignments(
    ...
    assignments = {a.event_type: a for a in db.query(SmsEventAssignment).all()}
    result = []
...
def assign_template_to_event(
    ...
    if payload.template_id is not None:
        tmpl = db.query(SmsTemplate).filter(SmsTemplate.id == payload.template_id).first()
        if not tmpl:
            raise HTTPException(
                status_code=404,
