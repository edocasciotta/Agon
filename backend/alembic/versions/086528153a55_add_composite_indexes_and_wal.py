...
def upgrade() -> None:
    op.create_index(
        "idx_booking_class_status", "bookings", ["scheduled_class_id", "status"], unique=False
    )
    op.create_index("idx_booking_client_status", "bookings", ["client_id", "status"], unique=False)
    op.create_index(
        "idx_membership_client_status", "memberships", ["client_id", "status"], unique=False
    )
    op.create_index(
        "idx_class_starts_at_location_status",
        "scheduled_classes",
        ["starts_at", "location_id", "status"],
        unique=False,
    )
