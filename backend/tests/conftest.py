...
def full_class_fixture(db_session, manager_user):
    ...
    tmpl = ClassTemplate(
        name="Pilates", duration_minutes=45, default_capacity=1, color="#000000", is_active=True
    )
    db_session.add(tmpl)
    db_session.commit()
    future = datetime.datetime.utcnow() + datetime.timedelta(hours=24)
    ...
    db_session.commit()
    b = Booking(
        client_id=filler.id, scheduled_class_id=sc.id, status="confirmed", credit_deducted=False
    )
    db_session.add(b)
    db_session.commit()
    db_session.refresh(sc)
