...
def test_preview_empty_filters_returns_all_clients(client, manager_auth_headers, db_session):
    ...
    c1 = Client(email="alice@example.com", password_hash=hash_password("pass1234"), full_name="Alice")
...
def test_preview_active_membership_filter(client, manager_auth_headers, db_session):
    ...
    c_active = Client(email="active@example.com", password_hash=hash_password("pass1234"), full_name="Active Member")
    c_no_mem = Client(email="nomem@example.com", password_hash=hash_password("pass1234"), full_name="No Membership")
...
def test_preview_not_booked_filter(client, manager_auth_headers, db_session):
    ...
    c_inactive = Client(email="inactive@example.com", password_hash=hash_password("pass1234"), full_name="Inactive")
    c_recent = Client(email="recent@example.com", password_hash=hash_password("pass1234"), full_name="Recent Booker")
    ...
    b = Booking(client_id=c_recent.id, scheduled_class_id=sc.id, status="confirmed", credit_deducted=False)
    db_session.add(b)
    db_session.commit()
