...
def test_create_client_from_backoffice(client, manager_auth_headers, studio_settings):
    ...
    with patch("app.services.email_service.aiosmtplib.send", new_callable=AsyncMock) as mock_send:
        ...
        resp = client.post(
            "/api/v1/clients",
            json={"full_name": "New Client", "email": "newclient@example.com", "phone": "+39123456789"},
...
def test_create_client_from_backoffice_email_sent_true(client, manager_auth_headers, studio_settings, db_session):
...
def test_create_client_duplicate_email(client, manager_auth_headers, studio_settings, registered_client):
...
def test_reset_password_flow(client, studio_settings, db_session):
    ...
    inv = db_session.query(InvitationToken).filter_by(client_id=client_obj.id).order_by(InvitationToken.id.desc()).first()
    assert inv is not None
    token_str = inv.token

    # Reset password
    resp3 = client.post("/api/v1/auth/reset-password", json={
        "token": token_str,
    ...
    resp4 = client.post("/api/v1/auth/login", json={"email": "resetme@example.com", "password": "oldpassword1"})
    assert resp4.status_code == 401

    # Verify new password works
    resp5 = client.post("/api/v1/auth/login", json={"email": "resetme@example.com", "password": "newpassword99"})
...
def test_reset_password_too_short(client, studio_settings, db_session):
    ...
    db_session.add(c)
    db_session.flush()
    inv = InvitationToken(client_id=c.id, token="short-token-abc", expires_at=utcnow() + timedelta(hours=1))
    db_session.add(inv)
    db_session.commit()
...
def test_reset_password_token_already_used(client, studio_settings, db_session):
    ...
    db_session.add(c)
    db_session.flush()
    inv = InvitationToken(client_id=c.id, token="used-token-xyz", used=True, expires_at=utcnow() + timedelta(hours=1))
    db_session.add(inv)
    db_session.commit()
