...
### Fixed
- Mobile app crashed immediately in Expo Go, before the login screen ever rendered ("Something went wrong"). `mobile/package.json` pinned `expo-font` (`^57.0.0`) and `expo-splash-screen` (`^57.0.2`) to versions incompatible with the project's Expo SDK 54 — a mismatch present since the project's original scaffolding, not a regression. `app/_layout.tsx` calls `SplashScreen.preventAutoHideAsync()` at module top-level, before any component renders, so Expo Go's SDK-54-native `expo-splash-screen` module (expecting the `~31.x` JS API) rejected the installed `57.x` JS package immediately on startup — the earliest possible point of failure. Also added `expo-constants`, which was missing entirely as a direct dependency despite being a required peer dependency of `expo-router`, and de-duplicated a stray `react-native-svg` version drift (`15.15.5` installed vs `15.12.1` expected). Fixed via `npx expo install expo-constants expo-font expo-splash-screen react-native-svg`, which resolved to `expo-constants@~18.0.13`, `expo-font@~14.0.12`, `expo-splash-screen@~31.0.13`, `react-native-svg@15.12.1`. `npx expo-doctor` now reports 18/18 checks passed (previously 3 failures).
- Desktop Reports page showed an inflated check-in rate (e.g. "10000.0%" instead of "100.0%") and a similarly inflated retention rate. The backend (`backend/app/routers/reports.py`) already returns `checkin_rate`/`retention_rate` as 0-100 scaled percentages, but `frontend/src/renderer/src/pages/Reports.tsx` multiplied them by 100 again before formatting. Removed the extra `* 100` at both StatCards (attendance tab check-in rate, retention tab retention rate). New regression test `frontend/tests/unit/pages/Reports.test.tsx` mocks the reports API and asserts the exact rendered string (`"100.0%"` / `"75.5%"`), not the 100x-inflated value.

### Changed
- Mobile appointment booking wizard's instructor step now actually filters by service eligibility instead of listing every instructor unconditionally. `mobile/app/appointment/book.tsx`'s instructor query is keyed on the selected service (`['available-instructors', serviceId]`, `enabled: serviceId !== null`) and calls the new `instructorsApi.listAvailableForService(serviceId)` (`mobile/src/api/instructors.ts`), which hits `GET /api/v1/appointment-services/{service_id}/available-instructors` (added in the service/establishment-scoping work above) instead of the unconditional `GET /api/v1/instructors`. The existing "No instructors are available for this service yet." empty state is now semantically accurate rather than only reflecting a studio with zero instructors overall. `mobile/__tests__/bookAppointment.test.tsx` updated to mock the new endpoint and gained a test asserting only eligible instructors render for the selected service, plus an assertion that the empty-state endpoint call is scoped to the right service id.
- Mobile membership purchase flow split into a compact list + dedicated checkout screen. `mobile/app/membership/purchase.tsx` now renders one small summary card per membership type (name, price with intro-offer badge, one-line unlimited/credits + type metadata) that navigates to a new `mobile/app/membership/checkout/[typeId].tsx` on tap instead of embedding a full promo-code input, gift-card input, and purchase button in every list item. The checkout screen owns the full pricing breakdown, promo/gift-card validate-apply-remove flow, and the Stripe checkout session purchase button for the single selected type — the same logic previously duplicated across every card, now scoped to one type (dropped the `*AppliedForTypeId` tracking needed only because the old screen held N cards at once). New `membership.checkout` / `membership.typeNotFound` i18n keys in all 7 locales. Tests split accordingly: `mobile/__tests__/purchase.test.tsx` now covers the compact list + navigation, `mobile/__tests__/membershipCheckout.test.tsx` covers the checkout screen's promo/gift-card/purchase behavior.
...
### Added
- Desktop Appointments settings now expose the service/establishment scoping added in PR #37: the Availability tab's "add window" form (`frontend/src/renderer/src/pages/Appointments/AvailabilityTab.tsx`) gained a per-day service `<select>` (default "All services", listing every active service from `appointmentServicesApi.list()`) that sends `service_id: null` when left on the default or the chosen service's id otherwise; each existing availability window now shows a badge ("All services" or the specific service name) next to its time range. The Services tab's create/edit modal (`frontend/src/renderer/src/pages/Appointments/ServicesTab.tsx`) gained an establishment checkbox list (sourced from `locationsApi.list()`) with "Leave empty to offer this service at all locations" helper text; the selection is always sent as `establishment_ids` (empty array = wildcard, matching the backend's replace-on-provided/untouched-on-omitted convention for `PATCH`). The services table gained a "Locations" column summarizing the linked establishment names or "All locations". New types (`AppointmentService.establishment_ids`, `InstructorAvailability.service_id`, and the matching `*Create`/`*Update` fields) in `frontend/src/renderer/src/types/index.ts`, updated Zod schemas in `frontend/src/renderer/src/lib/formSchemas.ts`. New `appointmentServices.establishments`/`establishmentsHelp`/`allLocations` and `instructorAvailability.service`/`allServices` i18n keys in all 7 locales. Updated/added Vitest coverage in `frontend/tests/unit/pages/Appointments/{AvailabilityTab,ServicesTab}.test.tsx`.
- Mobile instructor login: the mobile app previously only supported client/member login (`app/onboarding/login.tsx` always routed to `(tabs)`, the client tab shell). It now branches on the JWT `role` returned by `/auth/me` — instructors are routed to a new, entirely separate `app/(instructor-tabs)/` route group (Schedule, Profile) instead of reusing/modifying the client tabs, and push-token registration is skipped for non-client roles (`PUT /clients/me/push-token` is gated server-side on `get_current_client`, so an instructor token would 403 on every login otherwise). The Schedule tab (`app/(instructor-tabs)/schedule.tsx`) resolves the caller's own `Instructor.id` via `GET /api/v1/instructors/me`, fetches `GET /classes?instructor_id=...` for a 90-day window, and splits into Upcoming/Past sections (booking_count/capacity, status badge); tapping a class opens a new roster screen (`app/instructor-class/[id].tsx`) that merges `GET /classes/{id}/roster` with `GET /checkins/class/{id}` by `booking_id` to show each client's checked-in state, with a manual check-in button (`POST /checkins` with `method: 'manual'`, new `checkinsApi.manual`) and a confirmed "Mark Complete" action (`POST /classes/{id}/complete` — the backend has no double-completion guard, so the confirmation dialog is a client-side safety net). The Profile tab (`app/(instructor-tabs)/profile.tsx`) shows the instructor's own name/bio/email/photo (reusing the client profile screen's authenticated `expo-image` pattern) with photo upload wired up (`POST /instructors/{id}/photo`) and sign-out. `mobile/src/types/index.ts` gained `instructor_id`/`booking_count` on `ScheduledClass`, `photo_url` on `Instructor`, and new `RosterEntry`/`Checkin` types. New `mobile/src/api/checkins.ts`. New `CHECKIN_*` error codes in `errorMessages.ts`. New `instructorTabs.*`/`instructorSchedule.*`/`instructorClassDetail.*` i18n keys across all 7 locales. New tests: `loginRouting.test.tsx`, `instructorSchedule.test.tsx`, `instructorClassDetail.test.tsx`, `instructorProfile.test.tsx`. (`instructorsApi.getMe()` initially shipped as a client-side email-search workaround since `GET /instructors/me` didn't exist yet on `main` at the time; switched to call the real endpoint directly once the entry below merged.)
- `GET /api/v1/instructors/me`: lets a logged-in instructor discover their own `Instructor.id` (mobile "my teaching schedule" needs this before it can call `GET /classes?instructor_id=...`, which already existed). Resolves the JWT `sub` to the caller's `Instructor` row via `Instructor.user_id`, never by trusting `sub` as an `Instructor.id` directly (`User.id`/`Client.id` share an integer space per `docs/SECURITY_GUIDELINES.md` §1.1) — role is checked (`role == "instructor"`) before the DB lookup, so manager and client tokens get a 403 (`AUTH_INSUFFICIENT_PERMISSIONS`), not just a wrong-entity result. Returns the same shape as `GET /instructors/{id}`. The edge case of an instructor-role `User` with no linked `Instructor` row (shouldn't happen in practice) returns 404 (`INSTRUCTOR_PROFILE_NOT_FOUND`) instead of crashing. `backend/app/routers/instructors.py`; new tests in `backend/tests/test_authorization.py`. Confirmed as part of this change: `POST /classes/{id}/complete` (mark class completed) already uses `get_current_user` (manager or instructor) matching its documented "manager/instructor" access in `docs/TECHNICAL_SPEC.md` §6, and `POST /checkins` (`method: manual`) / `GET /checkins/class/{class_id}` already allow any non-client role — so instructor check-in and class-completion from mobile need no backend authorization change, only this new discovery endpoint.
- Service and establishment scoping for the appointment engine: `instructor_availability` gained a nullable `service_id` FK (`NULL` = available for ALL services, the wildcard every existing row implicitly means, so no backfill needed) and a new `appointment_service_locations` many-to-many join table links `AppointmentService` to `Location` (zero links = offered at ALL establishments, same wildcard convention). `POST`/`PATCH /api/v1/appointment-services` accept an optional `establishment_ids` list (`PATCH` replaces the entire linked set, it does not merge); responses now include `establishment_ids`. `POST /api/v1/instructor-availability` accepts an optional `service_id`. Availability lookups (`GET /api/v1/appointments/available-slots`, `POST /api/v1/appointments`) now respect service scoping (`service_id IS NULL OR service_id = :requested`). New `GET /api/v1/appointment-services/{service_id}/available-instructors` returns instructors eligible for a service (matching availability scoping and, when the service is establishment-scoped, matching the instructor's own location) without date filtering — mobile's instructor-picker step can now use this instead of listing every instructor unconditionally. New Alembic migration `aea4bdb031a8_add_service_scoping_to_availability_and_`. `backend/app/models/instructor_availability.py`, `backend/app/models/appointment_service_location.py` (new), `backend/app/models/__init__.py`, `backend/app/schemas/appointment_service.py`, `backend/app/schemas/instructor_availability.py`, `backend/app/routers/appointment_services.py`, `backend/app/routers/instructor_availability.py`, `backend/app/routers/appointments.py`, `backend/app/services/appointment_service.py`; new tests in `backend/tests/test_appointments.py`.
- Desktop profile photo upload for Instructors and Clients: a "Change photo" control (client-side validated for `.jpg/.jpeg/.png/.webp`, 5MB max, before any request is made) on the Instructors edit modal and the Client Detail page uploads via `POST /api/v1/instructors/{id}/photo` / `POST /api/v1/clients/{client_id}/photo` (`instructorsApi.uploadPhoto` / `clientsApi.uploadPhoto`, new in `frontend/src/renderer/src/api/instructors.ts` / `clients.ts`) and immediately invalidates the relevant query so the new photo shows without a manual refresh. Since `GET /api/v1/photos/{filename}` requires the manager's JWT (a plain `<img src>` can't attach the Authorization header), photos are fetched as a blob via the authenticated `apiClient` and rendered through an object URL by a new reusable `frontend/src/renderer/src/components/AuthenticatedImage.tsx` (revokes the object URL on unmount/`src` change), wrapped together with the upload control and validation in `frontend/src/renderer/src/components/PhotoUpload.tsx`. Falls back to the existing initials avatar (Instructors) / a generic person icon (Client Detail) when no photo is set. Translated across all 7 locales (`photoUpload.*`).
...
- Mobile profile photo upload: tapping the avatar on the Profile tab (`app/(tabs)/profile.tsx`) now opens the native photo library (`expo-image-picker`, media-library-only — camera/microphone permissions declined via `app.json`'s `expo-image-picker` plugin config) and uploads the selection via a new `clientsApi.uploadPhoto(clientId, asset)` (`mobile/src/api/memberships.ts`), which builds a React Native `{ uri, name, type }` multipart body for `POST /api/v1/clients/{client_id}/photo`. Oversized files (>5MB) are rejected client-side using the picker asset's `fileSize` before any upload attempt, mirroring the desktop app's pre-flight validation against the same backend limit. The photo is displayed with `expo-image`'s `source={{ uri, headers }}` (passing the JWT as an `Authorization` header) rather than a blob/base64 fallback, since `expo-image` already supports authenticated remote sources directly. A pencil badge overlays the avatar; a spinner replaces it while the upload is in flight; failures surface a friendly, translated inline message via the existing `getErrorMessage`/`errorMessages.ts` pattern (extended with `FORBIDDEN` and the backend's `PHOTO_*` codes) without blocking the rest of the screen. `ClientUser` (`mobile/src/types/index.ts`) gained `photo_url: string | null`. New strings in all 7 locales.
